...
<style>
  *{margin:0;padding:0;box-sizing:border-box} body{font-family:DejaVu Sans,sans-serif;font-size:11px;color:#1f2937}
  .header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:14px;border-bottom:2px solid #059669}
  .logo{font-size:18px;font-weight:700;color:#059669} .logo span{color:#1f2937}
  ...
  thead th{background:#065f46;color:#fff;padding:8px 10px;text-align:left;font-size:9px;text-transform:uppercase}
...
<div class="summary-grid">
  <div class="summary-card">
    ...
    <div class="value" style="color:#059669">{{ $purchases->count() }}</div>
  </div>
  <div class="summary-card">
...
<table>
  <thead>
    <tr><th>#</th><th>Reference</th><th>Supplier</th><th>Date</th><th>Total</th><th>Paid</th><th>Due</th><th>Status</th><th>Payment</th></tr>
  </thead>
  <tbody>
    @forelse($purchases as $i => $p)
    <tr>
      <td>{{ $i+1 }}</td>
      <td style="font-family:monospace;color:#059669">{{ $p->reference }}</td>
      <td>{{ $p->supplier->name }}</td>
      <td>{{ $p->purchase_date->format('d M Y') }}</td>
      <td style="font-weight:600">PKR {{ number_format($p->total_amount,2) }}</td>
      <td style="color:#059669">PKR {{ number_format($p->paid_amount,2) }}</td>
      <td style="color:#dc2626">PKR {{ number_format($p->due_amount,2) }}</td>
      <td><span class="badge-{{ $p->status==='received'?'success':'warning' }}">{{ ucfirst($p->status) }}</span></td>
      <td><span class="badge-{{ $p->payment_status==='paid'?'success':($p->payment_status==='partial'?'warning':'danger') }}">{{ ucfirst($p->payment_status) }}</span></td>
    </tr>
    @empty
...
<div class="footer">WMS Pro — Purchase Report | {{ now()->format('d M Y H:i') }}</div>
</body>
</html>
